...
// Cores das barras por tipo (conforme anexo)
const TIPO_COLORS = {
  loja:         { main: '#E8B432', light: '#F5D07A', bg: '#E8B43220', bar: '#E8B432' }, // Dourado
  obra:         { main: '#4CAF50', light: '#81C784', bg: '#4CAF5020', bar: '#4CAF50' }, // Verde
  distribuidor: { main: '#29B6F6', light: '#90CAF9', bg: '#29B6F620', bar: '#29B6F6' }, // Azul ciano
};
...
// ── CLIENTE CARD ─────────────────────────────────────────────
function ClienteCard({ item, onPress, onCapturarGPS }) {
  ...
  return (
    <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
      <TouchableOpacity style={[cc.card, { borderColor: tc.main + '50', shadowColor: tc.main }]} onPress={onPress} activeOpacity={0.85}>
        ...
        {/* Barra colorida topo — cor do tipo */}
        ...
        <View style={cc.header}>
          ...
          {item.foto ? (
            ...
            <View style={[cc.iconWrap, { backgroundColor: tc.bg }]}>
              <Icon name={getTipoIcon(item.tipo)} size={20} color={tc.main} />
            </View>
          )}
        ...
        <View style={cc.infoGrid}>
          {item.endereco ? (
            <View style={cc.infoRow}><Icon name="location-on" size={12} color={SILVER_DARK} /><Text style={cc.infoText} numberOfLines={1}>{item.endereco}</Text></View>
          ) : null}
          {item.telefone1 ? (
            <View style={cc.infoRow}><Icon name="phone" size={12} color={SILVER_DARK} /><Text style={cc.infoText}>{item.contato1 ? `${item.contato1}: ` : ''}{item.telefone1}</Text></View>
          ) : null}
          {item.telefone2 ? (
            <View style={cc.infoRow}><Icon name="phone" size={12} color={SILVER_DARK} /><Text style={cc.infoText}>{item.contato2 ? `${item.contato2}: ` : ''}{item.telefone2}</Text></View>
          ) : null}
          {item.cnpj ? (
            <View style={cc.infoRow}><Icon name="business" size={12} color={SILVER_DARK} /><Text style={cc.infoText}>{item.cnpj}</Text></View>
          ) : null}
          <View style={cc.infoRow}>
            <Icon name={temLocalizacao ? 'gps-fixed' : 'gps-off'} size={12} color={temLocalizacao ? SUCCESS : SILVER_DARK + '80'} />
          ...
          <View style={cc.footerActions}>
            ...
            {!temLocalizacao && (
              <TouchableOpacity style={[cc.actionBtn, { backgroundColor: SUCCESS + '18', borderColor: SUCCESS + '55' }]} onPress={handleGPS} disabled={capturando} activeOpacity={0.8}>
                <Icon name={capturando ? 'gps-not-fixed' : 'add-location'} size={15} color={SUCCESS} />
            ...
            {temLocalizacao && (
              <TouchableOpacity style={[cc.actionBtn, { backgroundColor: '#4285F420', borderColor: '#4285F460' }]}
                ...
                <Icon name="map" size={13} color="#4285F4" />
              </TouchableOpacity>
            )}
            {/* Waze */}
            {temLocalizacao && (
      ...
      const html = `
        ...
        <body>
          ...
          <table>
            <thead>
              <tr>
                <th>Nome</th><th>CNPJ</th><th>Telefone(s)</th>
                <th>E-mail</th><th>Endereço</th><th>Fornecimento</th>
              </tr>
            </thead>
            <tbody>${linhas}</tbody>
          </table>
    ...
    <View style={ds.container}>
      ...
      <Animated.View style={[ds.header, { opacity: headerAnim, transform: [{ translateY: headerAnim.interpolate({ inputRange: [0, 1], outputRange: [-20, 0] }) }] }]}>
        <View style={ds.headerTop}>
          ...
          <TouchableOpacity style={ds.pdfBtn} onPress={exportarPDF} disabled={exportando} activeOpacity={0.8}>
            <Icon name="picture-as-pdf" size={16} color={DARK_BG} />
      ...
      <View style={ds.searchWrap}>
        <View style={ds.searchBox}>
          <Icon name="search" size={18} color={SILVER_DARK} />
          ...
          {searchText.length > 0 && <TouchableOpacity onPress={() => setSearchText('')}><Icon name="close" size={16} color={SILVER_DARK} /></TouchableOpacity>}
        </View>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={ds.emptyWrap}>
            <Icon name="people-outline" size={56} color={GOLD + '40'} />
      ...
      <TouchableOpacity style={ds.fab} onPress={() => { resetForm(); setModalVisible(true); }} activeOpacity={0.85}>
        <Icon name="add" size={28} color={DARK_BG} />
      </TouchableOpacity>
            ...
            <View style={ms.header}>
              <View style={ms.headerLeft}>
                <View style={[ms.headerIcon, gpsTag && { backgroundColor: SUCCESS }]}>
                  <Icon name={gpsTag ? 'gps-fixed' : editingCliente ? 'edit' : 'person-add'} size={20} color={DARK_BG} />
                </View>
                <View>
              ...
              <TouchableOpacity style={ms.closeBtn} onPress={() => { setModalVisible(false); setGpsTag(false); }}>
                <Icon name="close" size={20} color={SILVER} />
              </TouchableOpacity>
            </View>
            ...
            <ScrollView style={ms.body} showsVerticalScrollIndicator={false}>
              ...
              {gpsTag && (
                <View style={ms.gpsBanner}>
                  <Icon name="gps-fixed" size={18} color={SUCCESS} />
              ...
              <View style={ms.fotoRow}>
                ...
                <View style={ms.fotoBtns}>
                  ...
                  <TouchableOpacity style={[ms.fotoBtn, { borderColor: SILVER + '40' }]} onPress={tirarFoto} activeOpacity={0.8}>
                    <Icon name="camera-alt" size={16} color={SILVER} />
              ...
              <View style={ms.inputWrap}>
                <Icon name="person" size={16} color={SILVER_DARK} style={{ marginRight: 8 }} />
              ...
              <View style={ms.inputWrap}>
                <Icon name="business" size={16} color={SILVER_DARK} style={{ marginRight: 8 }} />
              ...
              <View style={ms.inputRow}>
                <View style={[ms.inputWrap, { flex: 1 }]}>
                  <Icon name="person-outline" size={14} color={SILVER_DARK} style={{ marginRight: 6 }} />
                ...
                <View style={[ms.inputWrap, { flex: 1 }]}>
                  <Icon name="phone" size={14} color={SILVER_DARK} style={{ marginRight: 6 }} />
              ...
              <View style={ms.inputWrap}>
                <Icon name="email" size={16} color={SILVER_DARK} style={{ marginRight: 8 }} />
              ...
              <View style={[ms.inputWrap, gpsTag && { borderColor: SUCCESS + '60', backgroundColor: SUCCESS + '10' }]}>
                <Icon name="location-on" size={16} color={gpsTag ? SUCCESS : SILVER_DARK} style={{ marginRight: 8 }} />
                <TextInput style={ms.input} placeholder="Rua, número - Cidade" placeholderTextColor={SILVER_DARK} value={formData.endereco} onChangeText={t => setFormData(p => ({ ...p, endereco: t }))} />
              </View>

              {/* ── GPS NO MODAL ── */}
              <TouchableOpacity style={ms.gpsModalBtn} onPress={capturarGPSModal} activeOpacity={0.8}>
                <Icon name="my-location" size={16} color={SUCCESS} />
                  ...
                  return (
                    <TouchableOpacity key={f} style={[ms.fornItem, ativo && ms.fornItemActive]} onPress={() => toggleFornecedor(f)} activeOpacity={0.8}>
                      <View style={[ms.fornCheck, ativo && { backgroundColor: GOLD, borderColor: GOLD }]}>
                        {ativo && <Icon name="check" size={12} color={DARK_BG} />}
                      </View>
              ...
              <TouchableOpacity style={[ms.saveBtn, loading && { opacity: 0.7 }]} onPress={salvarCliente} disabled={loading} activeOpacity={0.85}>
                <Icon name={editingCliente ? 'save' : 'person-add'} size={18} color={DARK_BG} style={{ marginRight: 8 }} />
              ...
              {editingCliente && (
                <TouchableOpacity style={ms.deleteBtn} onPress={() => deleteCliente(editingCliente.id)} activeOpacity={0.8}>
                  <Icon name="delete-forever" size={18} color={DANGER} style={{ marginRight: 8 }} />
