    ...
    <View style={cal.wrap}>
      ...
      <View style={cal.nav}>
        ...
        <View style={cal.navCenter}>
          <Text style={cal.navTitle}>{MESES[viewMonth]}</Text>
      ...
      <ShimmerLine color={GOLD + '50'} />
      <View style={[cal.row, { marginTop: 8 }]}>
        {DIAS_SEMANA_SHORT.map((d, i) => (
          <View key={d} style={cal.cell}>
            <Text style={[cal.dayLabel,
              i === 0 && { color: DANGER + '99' },
              i === 6 && { color: SILVER + '80' }]}>{d}</Text>
          </View>
        ))}
      </View>
        ...
        <View key={week} style={cal.row}>
          {cells.slice(week * 7, week * 7 + 7).map((day, idx) => {
            ...
            return (
              <TouchableOpacity
                key={idx}
                style={[cal.cell, isSel && cal.cellSelected, isToday && !isSel && cal.cellToday]}
                onPress={() => onDayPress(iso)} activeOpacity={0.75}>
                <Text style={[cal.dayNum,
                  isSel && { color: DARK_BG, fontWeight: 'bold' },
                  isToday && !isSel && { color: GOLD, fontWeight: '700' }]}>
                  {day}
                </Text>
                {marked && (
...
const cal = StyleSheet.create({
  wrap:         { backgroundColor: CARD_BG, borderRadius: 20, marginBottom: 14, borderWidth: 1, borderColor: GOLD + '28', overflow: 'hidden', shadowColor: GOLD, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.14, shadowRadius: 10, elevation: 5 },
  topAccent:    { height: 3, backgroundColor: GOLD },
  nav:          { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 12 },
  navBtn:       { width: 36, height: 36, borderRadius: 18, backgroundColor: CARD_BG2, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: GOLD + '30' },
  navCenter:    { alignItems: 'center' },
  navTitle:     { fontSize: 17, fontWeight: 'bold', color: SILVER_LIGHT, letterSpacing: 0.4 },
  navYear:      { fontSize: 11, color: SILVER_DARK, marginTop: 1 },
  row:          { flexDirection: 'row', paddingHorizontal: 6 },
  cell:         { flex: 1, alignItems: 'center', paddingVertical: 5, minHeight: 38 },
  cellSelected: { backgroundColor: GOLD, borderRadius: 10 },
  cellToday:    { borderWidth: 1.5, borderColor: GOLD + '70', borderRadius: 10, backgroundColor: GOLD + '10' },
  dayLabel:     { fontSize: 10, color: SILVER_DARK, fontWeight: '700', letterSpacing: 0.3 },
...
function ItemCard({ item, tipo, onPress, onDelete, onToggle }) {
  ...
  return (
    <TouchableOpacity
      style={[ic.card, { borderLeftColor: accent, opacity: item.concluido ? 0.6 : 1 }]}
      ...
      <View style={ic.row}>
        {tipo !== 'visita' ? (
          <TouchableOpacity onPress={() => onToggle(item)} style={ic.check}>
            <Icon
              ...
              size={20} color={item.concluido ? SUCCESS : SILVER_DARK} type="material" />
          </TouchableOpacity>
        ) : (
          <View style={[ic.iconWrap, { backgroundColor: accent + '22' }]}>
            <Icon name="event" size={16} color={accent} type="material" />
          </View>
        )}
        <View style={{ flex: 1 }}>
        ...
        <View style={ic.right}>
          ...
          <TouchableOpacity onPress={() => onDelete(item)} style={ic.del}>
            <Icon name="delete-outline" size={16} color={DANGER} type="material" />
          </TouchableOpacity>
        </View>
      </View>
    </TouchableOpacity>
...
const ic = StyleSheet.create({
  ...
  del:     { width: 26, height: 26, borderRadius: 8, backgroundColor: DANGER + '14', justifyContent: 'center', alignItems: 'center' },
});
...
export default function PlanejamentoScreen({ navigation, route }) {
  ...
  const markedDates = {};
  visitas.forEach(v   => { if (!markedDates[v.data]) markedDates[v.data] = {}; markedDates[v.data].visita   = true; });
  tarefas.forEach(t   => { if (!markedDates[t.data]) markedDates[t.data] = {}; markedDates[t.data].tarefa   = true; });
  lembretes.forEach(l => { if (!markedDates[l.data]) markedDates[l.data] = {}; markedDates[l.data].lembrete = true; });
  ...
  const salvarItem = async () => {
    ...
    try {
      ...
      if (editingItem) { await updateDoc(doc(db, colName, editingItem.id), dados); }
      else             { await addDoc(collection(db, colName), dados); }
  ...
  const deleteItem = (item) => {
    ...
    Alert.alert('Excluir', `Remover "${item.titulo}"?`, [
      ...
      { text: 'Excluir', style: 'destructive', onPress: async () => {
        await deleteDoc(doc(db, colName, item.id)); await loadAll();
      }},
    ]);
  };
  ...
  const toggleConcluido = async (item) => {
    ...
    try { await updateDoc(doc(db, colName, item.id), { concluido: !item.concluido }); await loadAll(); }
    catch (e) {}
  };
  ...
  return (
    <View style={ds.container}>
      ...
      <View style={ds.header}>
        <View style={ds.topAccent} />
        <View style={ds.headerRow}>
          <View style={ds.headerLeft}>
            <View style={ds.headerIconWrap}>
              <Icon name="event-note" size={22} color={DARK_BG} type="material" />
            </View>
            <View>
          ...
          <TouchableOpacity style={ds.addBtn} onPress={() => openModal(activeTab)} activeOpacity={0.85}>
            <Icon name="add" size={20} color={DARK_BG} type="material" />
            <Text style={ds.addBtnTxt}>Novo</Text>
          </TouchableOpacity>
        </View>
        <ShimmerLine color={GOLD} />
        {/* Mini KPI bar */}
        <View style={ds.todayBar}>
          <Icon name="today" size={12} color={SILVER_DARK} style={{ marginRight: 6 }} type="material" />
          <Text style={ds.todayLbl}>Hoje: </Text>
          {[
            { v: visitasDoDia.length,   label: 'visitas',   c: GOLD    },
            { v: tarefasDoDia.length,   label: 'tarefas',   c: SILVER  },
            { v: lembretesDoDia.length, label: 'lembretes', c: SUCCESS },
          ].map((k, i) => (
            <React.Fragment key={i}>
              {i > 0 && <Text style={ds.todaySep}>·</Text>}
              <Text style={[ds.todayNum, { color: k.c }]}>{k.v}</Text>
              <Text style={ds.todayLbl}> {k.label}</Text>
            </React.Fragment>
          ))}
        </View>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
        ...
        <View style={ds.section}>
          <CalendarioPuro selectedDate={selectedDate} onDayPress={setSelectedDate} markedDates={markedDates} />
        </View>

        {/* RESUMO DO DIA */}
        <View style={ds.section}>
          <View style={ds.diaCard}>
            <View>
              <Text style={ds.diaSemana}>{diaSemana.toUpperCase()}</Text>
              <Text style={ds.diaData}>{`${sd}/${sm}/${sy}`}</Text>
            </View>
            <View style={ds.diaStats}>
              {TABS.map(tab => (
                <View key={tab.key} style={ds.diaStat}>
                  <Text style={[ds.diaStatNum, { color: tab.key === 'visita' ? GOLD : tab.key === 'tarefa' ? SILVER : SUCCESS }]}>{tab.count}</Text>
                  <Text style={ds.diaStatLbl}>{tab.label}</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        {/* ABAS */}
        <View style={[ds.section, { flexDirection: 'row', gap: 8 }]}>
          {TABS.map(tab => {
            ...
            return (
              <TouchableOpacity
                key={tab.key}
                style={[ds.tab, active && { backgroundColor: accent + '18', borderColor: accent + '60', borderBottomWidth: 2.5, borderBottomColor: accent }]}
                onPress={() => setActiveTab(tab.key)} activeOpacity={0.8}>
                <Icon name={tab.icon} size={14} color={active ? accent : SILVER_DARK} type="material" />
          ...
          {listData.length === 0 ? (
            <View style={ds.empty}>
              <View style={ds.emptyIconWrap}>
                <Icon name={activeTab === 'visita' ? 'event-busy' : activeTab === 'tarefa' ? 'assignment' : 'notifications-off'} size={34} color={GOLD + '50'} type="material" />
              </View>
              <Text style={ds.emptyTitle}>Nenhum item neste dia</Text>
              <TouchableOpacity style={ds.emptyBtn} onPress={() => openModal(activeTab)}>
                <Icon name="add" size={13} color={GOLD} type="material" />
      ...
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={mo.overlay}>
          <View style={mo.sheet}>
            <View style={mo.topAccent} />
            <View style={mo.header}>
              <View style={mo.headerLeft}>
                <View style={mo.headerIcon}>
                  <Icon name={modalType === 'visita' ? 'event' : modalType === 'tarefa' ? 'check-circle' : 'notifications'} size={18} color={DARK_BG} type="material" />
                </View>
            ...
            <ScrollView style={mo.body} showsVerticalScrollIndicator={false}>
              ...
              <View style={mo.inputWrap}>
                <TextInput style={mo.input} placeholder="Título" placeholderTextColor={SILVER_DARK}
                  value={formData.titulo} onChangeText={t => setFormData(p => ({ ...p, titulo: t }))} />
              </View>

              <Text style={mo.label}>DESCRIÇÃO</Text>
              <View style={[mo.inputWrap, { alignItems: 'flex-start', paddingTop: 10 }]}>
                <TextInput style={[mo.input, { height: 60, textAlignVertical: 'top' }]}
                  placeholder="Descrição opcional" placeholderTextColor={SILVER_DARK}
                  value={formData.descricao} onChangeText={t => setFormData(p => ({ ...p, descricao: t }))} multiline />
              </View>
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <View style={{ flex: 1 }}>
                  ...
                  <View style={mo.inputWrap}>
                    <TextInput style={mo.input} placeholder="AAAA-MM-DD" placeholderTextColor={SILVER_DARK}
                      value={formData.data} onChangeText={t => setFormData(p => ({ ...p, data: t }))} />
                  </View>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={mo.label}>HORA</Text>
                  <View style={mo.inputWrap}>
                    <TextInput style={mo.input} placeholder="09:00" placeholderTextColor={SILVER_DARK}
                      value={formData.hora} onChangeText={t => setFormData(p => ({ ...p, hora: t }))} />
                  </View>
                </View>
              </View>
              ...
              {modalType === 'visita' && (
                <>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
                    <TouchableOpacity style={[mo.chip, !formData.clienteId && mo.chipActive]}
                ...
                <>
                  ...
                  <View style={mo.optRow}>
                    {['baixa','media','alta'].map(p => (
                      <TouchableOpacity key={p} style={[mo.opt, { flex: 1 },
                        formData.prioridade === p && { backgroundColor: getPriorityColor(p) + '25', borderColor: getPriorityColor(p) + '80' }]}
              ...
              <TouchableOpacity style={[mo.saveBtn, loading && { opacity: 0.7 }]} onPress={salvarItem} disabled={loading} activeOpacity={0.85}>
                <Icon name="save" size={18} color={DARK_BG} style={{ marginRight: 8 }} type="material" />
...
const ds = StyleSheet.create({
  ...
  header:        { backgroundColor: '#001828', borderBottomLeftRadius: 24, borderBottomRightRadius: 24, overflow: 'hidden', shadowColor: GOLD, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.15, shadowRadius: 12, elevation: 8 },
  topAccent:     { height: 3, backgroundColor: GOLD },
  headerRow:     { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingTop: 16, paddingBottom: 10 },
  headerLeft:    { flexDirection: 'row', alignItems: 'center', gap: 12 },
  headerIconWrap:{ width: 42, height: 42, borderRadius: 14, backgroundColor: GOLD, justifyContent: 'center', alignItems: 'center' },
  headerTitle:   { fontSize: 22, fontWeight: 'bold', color: SILVER_LIGHT },
  headerSub:     { fontSize: 11, color: SILVER_DARK, marginTop: 1 },
  addBtn:        { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: GOLD, paddingHorizontal: 14, paddingVertical: 9, borderRadius: 12, shadowColor: GOLD, shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.45, shadowRadius: 6, elevation: 5 },
  addBtnTxt:     { fontSize: 13, fontWeight: 'bold', color: DARK_BG },
  todayBar:      { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 10 },
  todayLbl:      { fontSize: 11, color: SILVER_DARK },
  todayNum:      { fontSize: 12, fontWeight: 'bold' },
  todaySep:      { fontSize: 11, color: SILVER_DARK + '60', marginHorizontal: 4 },
  section:       { paddingHorizontal: 16, paddingTop: 14 },
  diaCard:       { backgroundColor: CARD_BG, borderRadius: 16, padding: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderWidth: 1, borderColor: GOLD + '28', shadowColor: GOLD, shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 3 },
  diaSemana:     { fontSize: 10, color: SILVER_DARK, fontWeight: '700', letterSpacing: 1.2 },
  diaData:       { fontSize: 22, fontWeight: 'bold', color: GOLD, marginTop: 2 },
  diaStats:      { flexDirection: 'row', gap: 20 },
  diaStat:       { alignItems: 'center' },
  diaStatNum:    { fontSize: 20, fontWeight: 'bold' },
  diaStatLbl:    { fontSize: 9, color: SILVER_DARK, marginTop: 1 },
  tab:           { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, paddingVertical: 10, borderRadius: 12, backgroundColor: CARD_BG, borderWidth: 1, borderColor: SILVER + '20' },
  ...
  emptyIconWrap: { width: 68, height: 68, borderRadius: 20, backgroundColor: GOLD + '10', justifyContent: 'center', alignItems: 'center', marginBottom: 12, borderWidth: 1, borderColor: GOLD + '28' },
  emptyTitle:    { fontSize: 13, color: SILVER_DARK, marginBottom: 14 },
  emptyBtn:      { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: GOLD + '18', borderRadius: 12, paddingHorizontal: 20, paddingVertical: 10, borderWidth: 1, borderColor: GOLD + '60' },
  emptyBtnTxt:   { fontSize: 12, color: GOLD, fontWeight: '700' },
});

const mo = StyleSheet.create({
  overlay:     { flex: 1, backgroundColor: 'rgba(0,0,0,0.75)', justifyContent: 'flex-end' },
  sheet:       { backgroundColor: MODAL_BG, borderTopLeftRadius: 28, borderTopRightRadius: 28, maxHeight: '90%', overflow: 'hidden', borderTopWidth: 1, borderColor: GOLD + '30' },
  topAccent:   { height: 3, backgroundColor: GOLD },
